...
void HomeScreen::drawUI() {
    ...
    if (!nameEntered) { // The state when client hasn't entered its user name.
        ...
        if ( connection == false ) {
            ...
            //if ( c < 0 ) {}// the part we have to deal with if the server is not on
            if ( c < 0 ) {
                std::cout << "Connection Error\n"; 
                exit(1); 
            }
            char name[20];
            std::string temp = playerNameText.getString();
            for (int i = 0; i < 20; ++i) { name[i] = temp[i]; }
            n = write(sockfd, name, strlen(name));
            n = read(sockfd, snd, sizeof(snd));
            connection = true;
        }
    	
        sprintf(snd, "%s", REQ_NUM);
        n = write(sockfd, snd, strlen(snd));
        n = read(sockfd, get, sizeof(get));
        int numOfOnlinePlayer, numOfPlayinPlayer;
        f = numOfOnlinePlayer = numOfPlayinPlayer = 0;
        for (int i = 0; i < n; ++i) {
            if ( isdigit(get[i]) ) {
                if ( f ) { numOfPlayinPlayer = numOfPlayinPlayer * 10 + (get[i] - '0'); }
                else { numOfOnlinePlayer = numOfOnlinePlayer * 10 + (get[i] - '0'); }
            } else { f++; }
        }

    	/*-----------------------------------------*/
    	//Need to update this number based on what it received from server.
    	onlinePlayerNum = numOfOnlinePlayer;
    	/*-----------------------------------------*/
    	...
    	/*-----------------------------------------*/
    	//Need to update this number based on what it received from server.
    	inGamePlayerNum = numOfPlayinPlayer;
    	/*-----------------------------------------*/
...
void HomeScreen::run() {
    ...
    if ( connection && gameStart == 0 ) {
        int c;
        c = write(sockfd, s, strlen(s));
        if ( c > 0 ) { std::cout << "close sent!\n"; }
        shutdown(sockfd, SHUT_WR);
    }
}


void HomeScreen::init(char *addr) {
